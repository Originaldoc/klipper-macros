...
echo 'G1 Z5' >> ${SD_PATH}/${FILENAME}
echo 'G90' >> ${SD_PATH}/${FILENAME}
echo 'G28 X Y' >> ${SD_PATH}/${FILENAME}
echo 'START_TEMPS' >> ${SD_PATH}/${FILENAME}
cat /tmp/plrtmpA.$$ | sed '/ Z'${1}'/q' | sed -ne '/\(M104\|M140\|M109\|M190\|M106\)/p' >> ${SD_PATH}/${FILENAME}
...
# cat /tmp/plrtmpA.$$ | sed -e '1,/ Z'${1}'[^0-9]*$/ d' | sed -e '/ Z/q' | tac | grep -m 1 ' E' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p' >> ${SD_PATH}/${FILENAME}
#tac /tmp/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]*$/q' | tac | tail -n+2 | sed -e '/ Z[0-9]/ q' | tac | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p' >> ${SD_PATH}/${FILENAME}
...
echo 'G91' >> ${SD_PATH}/${FILENAME}
echo 'G1 Z-5' >> ${SD_PATH}/${FILENAME}
echo 'G90' >> ${SD_PATH}/${FILENAME}
# cat /tmp/plrtmpA.$$ | sed -e '1,/Z'${1}'/ d' | sed -ne '/ Z/,$ p' >> ${SD_PATH}/${FILENAME}
tac /tmp/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]*$/q' | tac | tail -n+2 | sed -ne '/ Z/,$ p' >> ${SD_PATH}/${FILENAME}
rm /tmp/plrtmpA.$$
